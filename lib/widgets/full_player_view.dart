...
class _FullPlayerViewState extends State<FullPlayerView> {
  ...
  Future<void> _loadLyrics() async {
    ...
    try {
      ...
      // STEP 3: Fetch translation in background, then update UI
      _apiService.fetchTranslation(surah.number, arabicAyahs, edition: _settings.defaultTranslation).then((fullAyahs) {
  ...
  void _scrollToAyah(int ayahNumber) {
    ...
    if (index >= 0 && index < (_ayahs!.length + (_hasBismillah(surahNum) ? 1 : 0))) {
      try {
           ...
           SafeArea(
             ...
             child: Padding(
               ...
               child: Row(
                 ...
                 children: [
                   IconButton(
                     icon: Icon(Icons.keyboard_arrow_down, color: colorScheme.onSurface, size: 32),
                   ...
                   Column(
                     children: [
                       Text(
                         "NOW PLAYING",
                         style: GoogleFonts.spaceGrotesk(color: colorScheme.primary, fontSize: 10, letterSpacing: 1),
                       ),
                       Text(
                         surah.name,
                         style: GoogleFonts.spaceGrotesk(color: colorScheme.onSurface, fontWeight: FontWeight.bold, fontSize: 16),
                       ),
                     ],
                   ),
                   IconButton(
                     icon: Icon(Icons.translate, color: _showTranslation ? colorScheme.primary : colorScheme.onSurface),
                     onPressed: () {
                        setState(() { 
  ...
  Widget _buildBody() {
    final colorScheme = Theme.of(context).colorScheme;
    if (_loading) return Center(child: CircularProgressIndicator(color: colorScheme.primary));
      ...
      itemBuilder: (context, index) {
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isCurrent ? colorScheme.primary.withValues(alpha: 0.08) : Colors.transparent,
              border: Border(
              ...
              children: [
                ...
                if (_showTranslation && ayah.translation.isNotEmpty) ...[
                  ...
                  Text(
                    ...
                    style: GoogleFonts.spaceGrotesk(
                      fontSize: 16,
                      color: isCurrent ? colorScheme.onSurface.withValues(alpha: 0.7) : colorScheme.onSurface.withValues(alpha: 0.5),
                      height: 1.6,
                    ),
                  ),
                ]
  ...
  Widget _buildControls() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
         color: Theme.of(context).brightness == Brightness.dark ? const Color(0xFF0D0D0D) : const Color(0xFFF5F5F5),
             ...
             children: [
                TextButton.icon(
                   onPressed: (_audioService.currentAyah > 1) 
                       ? () => _audioService.playAyah(_audioService.currentSurah!, _audioService.currentAyah - 1) 
                       : null,
                   icon: Icon(Icons.skip_previous_rounded, color: (_audioService.currentAyah > 1) ? colorScheme.onSurface.withValues(alpha: 0.54) : colorScheme.onSurface.withValues(alpha: 0.1)),
                   label: Text(
                      "Prev Ayah", 
                      style: GoogleFonts.spaceGrotesk(
                         fontSize: 12, 
                         color: (_audioService.currentAyah > 1) ? colorScheme.onSurface.withValues(alpha: 0.54) : colorScheme.onSurface.withValues(alpha: 0.1)
                      )
                   ),
                ),
                TextButton.icon(
                   onPressed: (_audioService.currentSurah != null && _audioService.currentAyah < _audioService.currentSurah!.totalAyahs) 
                       ? () => _audioService.playAyah(_audioService.currentSurah!, _audioService.currentAyah + 1) 
                       : null,
                   label: Text(
                      "Next Ayah", 
                      style: GoogleFonts.spaceGrotesk(
                         fontSize: 12, 
                         color: (_audioService.currentSurah != null && _audioService.currentAyah < _audioService.currentSurah!.totalAyahs) ? colorScheme.onSurface.withValues(alpha: 0.54) : colorScheme.onSurface.withValues(alpha: 0.1)
                      )
                   ),
                   icon: Icon(Icons.skip_next_rounded, color: (_audioService.currentSurah != null && _audioService.currentAyah < _audioService.currentSurah!.totalAyahs) ? colorScheme.onSurface.withValues(alpha: 0.54) : colorScheme.onSurface.withValues(alpha: 0.1)),
                ),
             ],
           ),
            ...
            children: [
               ...
               IconButton(
                 ...
                 icon: Icon(
                    ...
                    color: _audioService.repeatMode == RepeatMode.none ? colorScheme.onSurface.withValues(alpha: 0.38) : colorScheme.primary,
                 ),
               ...
               IconButton(
                 iconSize: 24,
                 icon: Icon(Icons.skip_previous, color: _audioService.hasPrevSurah() ? colorScheme.onSurface : colorScheme.onSurface.withValues(alpha: 0.38)),
               ...
               Container(
                 decoration: BoxDecoration(
                   ...
                   borderRadius: BorderRadius.circular(99),
                   boxShadow: const [
                      BoxShadow(color: Color(0x4040B779), blurRadius: 20, offset: Offset(0, 4))
                   ]
                 ),
                 child: IconButton(
                   iconSize: 32,
               ...
               IconButton(
                 iconSize: 24,
                 icon: Icon(Icons.skip_next, color: _audioService.hasNextSurah() ? colorScheme.onSurface : colorScheme.onSurface.withValues(alpha: 0.38)),
               ...
               IconButton(
                 ...
                 icon: Icon(
                    Icons.sort_rounded,
                    color: _audioService.surahOrder == SurahOrder.ascending ? colorScheme.primary : Colors.amber, 
                 ),
